...
import os, sys, time, threading, subprocess, datetime, socket, platform, curses
...
def panel_sysinfo(info):
    now = datetime.datetime.now().strftime("%H:%M:%S")
    g = Table.grid(padding=(0,1))
    g.add_column(style="bright_cyan bold", min_width=8)
    g.add_column(min_width=26)
    rows = [
        ("HOST",   Text(info["hostname"], style="bold white")),
        ("USER",   Text(info["user"],     style="bright_green")),
        ("OS",     Text(info["os_str"],   style="dim white")),
        ("ARCH",   Text(info["arch"],     style="dim white")),
        ("PYTHON", Text(info["python"],   style="yellow")),
        ("SHELL",  Text(info["shell"],    style="dim white")),
...
def panel_scripts():
    ...
    tbl.add_column("SCRIPT", style="bold white",   min_width=13)
    tbl.add_column("DESC",   style="dim white",    min_width=20)
    tbl.add_column("RUNS",   style="bright_yellow",width=5,  justify="right")
    for i, (name, _, desc) in enumerate(SCRIPTS):
        sel = (i == S.selected)
        prefix = "▶ " if sel else "  "
        tbl.add_row(str(i+1), prefix+name, desc,
                    str(S.run_count[i]) if S.run_count[i] else "—",
                    style="on grey19" if sel else "")
...
def panel_log():
    t = Text()
    for ts, msg, style in S.log_lines[-12:]:
        t.append(f"[{ts}] ", style="dim")
        t.append(msg + "\n", style=style)
    return Panel(t, title="[bold bright_yellow]◈ ACTIVITY LOG[/]",
...
def panel_status():
    ...
    bar.add_row(
        Text(f"  ◈ MODE: [{mc}]{S.mode.upper()}[/{mc}]  |  TOTAL RUNS: {total}", style="bold"),
...
def build_layout(info):
    ...
    L["logo"].update(panel_logo())
    L["left"].update(panel_sysinfo(info))
    L["status"].update(panel_status())
    L["log"].update(panel_log())
    if S.mode in ("running", "output"):
        L["scripts"].update(panel_output())
    else:
        L["scripts"].update(panel_scripts())
    return L
...
# ── curses key reader (runs in its own thread, only reads keys) ───────────────
        ...
        if S.mode == "menu":
            if   key in (curses.KEY_UP,   ord('k'), ord('K')):
                ...
                if not any(t.name == "runner" for t in threading.enumerate()):
                    t = threading.Thread(target=run_script, args=(S.selected,), daemon=True, name="runner")
                    t.start()
...
def main():
    ...
    time.sleep(0.15)   # let curses initialise before Rich takes the screen
